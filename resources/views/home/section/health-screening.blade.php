<div class="max-w-screen-2xl mx-auto py-24">
    <div class="flex flex-col mb-8">
        <x-typography.subheading>Your Health, Your Priority</x-typography.subheading>
        <x-typography.heading>Discover Our Health Screening Package</x-typography.heading>
    </div>
    <x-slider id="screeningSlider" class="mb-12" arrow="bottom-right" items="4" infinity="true" centered="true">
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description="Recommended for age group from
            25-35 years old for both male and
            female"
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description="Recommended for age group from
            25-35 years old for both male and
            female"
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description="Recommended for age group from
            25-35 years old for both male and
            female"
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description="Recommended for age group from
            25-35 years old for both male and
            female"
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description=""
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description=""
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description=""
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description=""
            />
        </x-slider.slider-item>
        <x-slider.slider-item>
            <x-grid.basic
                image="{{asset('asset/ScreeningPackage/cover-1.jpg')}}"
                heading="General Health Screening Package"
                description="Recommended for age group from
            25-35 years old for both male and
            female"
            />
        </x-slider.slider-item>
    </x-slider>
    <x-button.link href="#">More Health Screening Package</x-button.link>

</div>
